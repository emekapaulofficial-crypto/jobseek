...
(function(){
  ...
  async function submitRequest(data){
    ...
    const row={
      ...
      service_type:data.serviceType||'APPLICATION_ADMINISTRATION',
      status:'REQUESTED'
    };
    const {data:created,error}=await c.from('jobseek_application_requests').insert(row).select('id,status,created_at').single();
    if(error) throw new Error(error.message||'Request could not be submitted.');
    return created;
  }
  async function listCandidateRequests(){
    ...
    const {data,error}=await c.from('jobseek_application_requests').select('*').eq('user_id',userData.user.id).order('created_at',{ascending:false});
