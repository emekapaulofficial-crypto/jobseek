...
def parse_date(value):
    ...
    if isinstance(value, (int, float)):
        try:
            ts=float(value); ts=ts/1000 if ts>10_000_000_000 else ts
...
def risk_for(title,description,company,url):
    hits=SCAM_TERMS.findall(f"{title} {description} {company} {url}")
    return ("high",sorted(set(hits))) if hits else ("low",[])

def normalize(source,item):
    if source.lower().replace(" ","") in EXCLUDED_SOURCES: return None
    ...
    company=clean(item.get("company") or item.get("companyName")); location=clean(item.get("location") or item.get("jobGeo") or "See listing")
    ...
    risk,risk_flags=risk_for(title,description,company,url); text=f"{title} {description} {location}"
    visa=bool(VISA_TERMS.search(text)) and not bool(NO_VISA_TERMS.search(text)); visa_evidence=VISA_TERMS.findall(text) if visa else []
...
def from_json(data,source):
    obj=json.loads(data); rows=obj.get("jobs",obj.get("results",[])) if isinstance(obj,dict) else obj
    return [j for row in rows or [] if isinstance(row,dict) and (j:=normalize(source,row))]
...
def adzuna():
    ...
    countries=os.getenv("ADZUNA_COUNTRIES","gb,us,ca,au,de,fr,nl,nz,sg,za,ng").split(","); queries=os.getenv("ADZUNA_QUERIES","construction,electrician,driver,warehouse,healthcare,engineering,technology,hospitality,cleaning,finance,sales").split(",")
    out=[]; errors=[]
...
def main():
    ...
    unique={}
    for job in jobs:
        if job.get("source","").lower().replace(" ","") in EXCLUDED_SOURCES: continue
