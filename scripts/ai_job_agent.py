...
"""
import hashlib, html, json, os, re, socket
...
NIGERIA_TERMS=re.compile(r"\b(nigeria|nigerian|lagos|abuja|port harcourt|ibadan)\b",re.I)
COUNTRY_LOCK_TERMS=re.compile(r"\b(must (currently )?(reside|live) in|residents? only|based in|located in|local bank account|must have (the )?right to work|must already have work authorization|legally eligible to work in|work authorization required|work permit required|only able to hire candidates who .* (reside|live)|country-specific work authorization)\b",re.I)
CATEGORY_RULES={
"Construction & Skilled Trades":["construction","electrician","welder","plumber","carpenter","mason","bricklayer","roofer","mechanic","scaffolder","laborer"],"Farming & Agriculture":["farm","agriculture","fruit picker","harvest","greenhouse","livestock","poultry","dairy","tractor","irrigation","horticulture","orchard","vineyard"],"Technology & IT":["software","developer","programmer","devops","cloud","cybersecurity","it support","data analyst","data scientist","qa","frontend","backend","full stack"],"Healthcare":["nurse","nursing","doctor","medical","healthcare","caregiver","pharmacy","dental"],"Engineering":["civil engineer","mechanical engineer","electrical engineer","chemical engineer","engineering"],"Driving & Transport":["driver","truck","delivery","transport","chauffeur"],"Logistics & Warehouse":["warehouse","logistics","supply chain","forklift","picker","packer"],"Hospitality & Catering":["hotel","hospitality","chef","cook","restaurant","catering","housekeeping"],"Cleaning & Facilities":["cleaner","cleaning","janitor","facilities","maintenance"],"Security":["security","guard","loss prevention"],"Finance & Accounting":["accountant","accounting","finance","auditor","bookkeeper"],"Administration":["administrator","administrative","office assistant","receptionist","hr assistant"],"Sales & Customer Service":["sales","customer service","customer support","account manager","call center"],"Marketing & Design":["marketing","seo","content","copywriter","graphic designer","designer","social media"],"Education":["teacher","tutor","lecturer","education","school"],"Retail":["retail","store","cashier","merchandiser"]}
COUNTRY_ALIASES={"Canada":["canada","ca"],"United Kingdom":["united kingdom","uk","england","scotland","wales","northern ireland","gb"],"United States":["usa","u.s.a","united states","us","america"],"Australia":["australia","au"],"Germany":["germany","deutschland","de"],"France":["france","fr"],"Netherlands":["netherlands","holland","nl"],"Ireland":["ireland","eire","ie"],"New Zealand":["new zealand","nz"],"Poland":["poland","pl"],"Sweden":["sweden","se"],"Norway":["norway","no"],"Denmark":["denmark","dk"],"Finland":["finland","fi"],"Switzerland":["switzerland","ch"],"Austria":["austria","at"],"Belgium":["belgium","be"],"Spain":["spain","es"],"Portugal":["portugal","pt"],"Italy":["italy","it"],"Greece":["greece","gr"],"Czechia":["czechia","czech republic","cz"],"Romania":["romania","ro"],"Hungary":["hungary","hu"],"South Africa":["south africa","za"],"Nigeria":["nigeria","ng"],"Ghana":["ghana","gh"],"Kenya":["kenya","ke"],"United Arab Emirates":["uae","united arab emirates","dubai","abu dhabi"],"Saudi Arabia":["saudi arabia","ksa","riyadh","jeddah"],"Qatar":["qatar","doha"],"Kuwait":["kuwait"],"Bahrain":["bahrain"],"Oman":["oman"],"Singapore":["singapore","sg"],"Japan":["japan","jp"],"South Korea":["south korea","korea","kr"],"India":["india","in"],"Philippines":["philippines","ph"],"Israel":["israel","il"],"Mexico":["mexico","mx"],"Brazil":["brazil","br"],"Argentina":["argentina","ar"],"Chile":["chile","cl"],"Colombia":["colombia","co"]}
...
def fetch(url):
 ...
 with urlopen(req,timeout=30) as response:return response.read(),response.geturl()
def check_url(url):
 try:
  parsed=urlparse(url)
  if parsed.scheme not in {"http","https"} or not parsed.hostname:return False,"invalid_url"
  socket.gethostbyname(parsed.hostname); req=Request(url,headers={"User-Agent":USER_AGENT},method="HEAD")
  with urlopen(req,timeout=12) as response:return 200<=response.status<400,str(response.status)
 except Exception:
  try:
...
def parse_date(value):
 ...
 if isinstance(value,(int,float)):
  try:
   ts=float(value); ts=ts/1000 if ts>10_000_000_000 else ts; return datetime.fromtimestamp(ts,tz=timezone.utc)
  except (OverflowError,OSError,ValueError):return None
 text=clean(value)
 for fmt in ("%Y-%m-%dT%H:%M:%SZ","%Y-%m-%dT%H:%M:%S%z","%Y-%m-%d","%a, %d %b %Y %H:%M:%S %z"):
  try:
   dt=datetime.strptime(text,fmt); return (dt if dt.tzinfo else dt.replace(tzinfo=timezone.utc)).astimezone(timezone.utc)
  except ValueError:pass
 try:return parsedate_to_datetime(text).astimezone(timezone.utc)
 except (TypeError,ValueError,OverflowError):return None
def risk_for(title,description,company,url):
 hits=SCAM_TERMS.findall(f"{title} {description} {company} {url}"); return ("high",sorted(set(hits))) if hits else ("low",[])
...
def normalize(source,item):
 ...
 title=clean(item.get("title") or item.get("jobTitle") or item.get("name")); url=clean(item.get("url") or item.get("link"))
...
def from_rss(data,source):
 ...
 for node in root.findall(".//item"):
  j=normalize(source,{"title":node.findtext("title"),"link":node.findtext("link"),"description":node.findtext("description"),"pubDate":node.findtext("pubDate"),"location":node.findtext("location")})
  if j:out.append(j)
 return out
def remoteok():
 data,_=fetch("https://remoteok.com/api"); out=[]
 for x in json.loads(data):
  if isinstance(x,dict) and x.get("id") and x.get("position"):
   j=normalize("RemoteOK",{"title":x.get("position"),"company":x.get("company"),"location":x.get("location") or "Remote","description":x.get("description"),"url":x.get("url") or ("https://remoteok.com/"+str(x.get("slug",""))),"published_at":x.get("date")})
   if j:out.append(j)
 return out
def remotive():data,_=fetch("https://remotive.com/api/remote-jobs?limit=200"); return from_json(data,"Remotive")
def adzuna():
 ...
 for country in countries:
  for query in queries:
   try:
    params=urlencode({"app_id":aid,"app_key":key,"results_per_page":50,"what":query.strip(),"content-type":"application/json"}); data,_=fetch(f"https://api.adzuna.com/v1/api/jobs/{country.strip()}/search/1?{params}")
    for x in json.loads(data).get("results",[]):
     j=normalize("Adzuna",{"title":x.get("title"),"description":x.get("description"),"url":x.get("redirect_url"),"published_at":x.get("created"),"location":(x.get("location") or {}).get("display_name") or country,"company":(x.get("company") or {}).get("display_name")})
     if j:out.append(j)
...
def verify_job(job,check_urls=True):
 ...
 closing=parse_date(job.get("closing_date"))
 if closing and closing<=now:reasons.append("expired")
 job["verification_checked_at"]=now.isoformat(); job["verification_reasons"]=sorted(set(reasons))
 if "expired" in reasons or job.get("risk_level")=="high":job["verification_status"]="REMOVED"
 elif not reasons and job.get("source_trusted") and job.get("apply_url_checked"):job["verification_status"]="VERIFIED"
 else:job["verification_status"]="NOT CONFIRMED"
 return job
def main():
 ...
 for name,loader in (("RemoteOK",remoteok),("Remotive",remotive)):
  try:jobs.extend(loader())
  except Exception as exc:errors.append({"source":name,"error":str(exc)})
 extra,extra_errors=adzuna(); jobs.extend(extra); errors.extend(extra_errors); unique={}
 for job in jobs:
